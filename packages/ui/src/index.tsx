...
export function PixelPanel({ children, className = '', title, action }: PropsWithChildren<{ className?: string; title?: string; action?: ReactNode }>) {
  return (
    <section className={`pixel-panel chronicle-section ${className}`.trim()}>
      {(title !== undefined || action !== undefined) && <header className="pixel-panel__header"><span className="section-mark" aria-hidden="true">◆</span>{title !== undefined && <h2>{title}</h2>}{action}</header>}
      {children}
    </section>
  );
}
...
export function DataBadge({ tone, children }: PropsWithChildren<{ tone: 'live' | 'cache' | 'fixture' | 'warning' | 'offline' }>) {
  return <span className={`data-badge data-badge--${tone}`}><span aria-hidden="true">●</span>{children}</span>;
}
