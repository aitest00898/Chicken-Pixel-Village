...
export function HousesPage({ village, online }: { village: VillageState; online: boolean }) {
  ...
  function submitHouse(event: FormEvent<HTMLFormElement>) {
    ...
    const house: ChickenHouse = { id: crypto.randomUUID(), organizationId: 'org-demo', revision: 0, createdAt: now, updatedAt: now, deletedAt: null, deviceId: 'local-device', operationId: crypto.randomUUID(), syncStatus: 'pending', name: String(data.get('name')), species: String(data.get('species')) as ChickenHouse['species'], designCapacity: Number(data.get('capacity')), currentBirdCount: 0, fosterFarmerId: null, archivedAt: null };
    village.addHouse(house); setSelected(house.id); event.currentTarget.reset(); setShowForm(false); setMessage('雞舍草稿已保存在本機並排入同步。');
  }
  ...
  return <div className="page houses-page">
    ...
    {!online && <div className="offline-draft-note" role="status">目前離線：雞舍、批次、風險與地圖可保存本機草稿；分潤確認、付款、沖銷及正式結算需連線。</div>}
    ...
    {showForm && <form className="house-form" onSubmit={submitHouse}><label>雞舍名稱<input required name="name" maxLength={30} /></label><SpeciesSelect /><label>設計容量<input required name="capacity" type="number" min="1" max="1000000" /></label><button className="primary-button" type="submit">保存本機草稿</button><small>容量會決定村莊中的建築規模。</small></form>}
    ...
    {selectedHouse && <PixelPanel className="house-detail">
      ...
      <div className="detail-tabs" role="tablist">{([['overview', '總覽'], ['batches', '批次'], ['shareholders', '股東'], ['distributions', '分潤'], ['risk', '風險'], ['edit', '編輯']] as Array<[Tab, string]>).map(([id, label]) => <button role="tab" aria-selected={tab === id} className={tab === id ? 'active' : ''} onClick={() => setTab(id)} key={id}>{label}</button>)}</div>
      {tab === 'overview' && <Overview houseId={selectedHouse.id} village={village} />}
      {tab === 'batches' && <Batches house={selectedHouse} village={village} onMessage={setMessage} />}
      {tab === 'shareholders' && <Shareholders houseId={selectedHouse.id} village={village} onMessage={setMessage} />}
      {tab === 'distributions' && <Distributions houseId={selectedHouse.id} village={village} online={online} run={run} />}
      {tab === 'risk' && <RiskForm houseId={selectedHouse.id} village={village} onMessage={setMessage} />}
      {tab === 'edit' && <EditHouse house={selectedHouse} village={village} onMessage={setMessage} />}
    </PixelPanel>}
  </div>;
}
...
function Batches({ house, village, onMessage }: { house: ChickenHouse; village: VillageState; onMessage: (message: string) => void }) {
  ...
  return <div className="operation-stack">{rows.map((row) => <div className="batch-card" key={row.id}><span className="status-light" /><div><strong>{row.batchCode}</strong><small>{row.placementDate} 入雛・{row.currentCount.toLocaleString()} 隻・預售 {row.expectedSaleDate ?? '未定'}</small></div><b>{row.status === 'draft' ? '草稿' : row.status}</b></div>)}<form className="inline-operation-form" onSubmit={submit}><h3>新增批次草稿</h3><label>批次代碼<input required name="code" /></label><label>入雛數<input required min="1" type="number" name="count" /></label><label>入雛日<input required type="date" name="placementDate" /></label><label>預計出貨<input type="date" name="saleDate" /></label><button className="secondary-button">保存草稿</button></form></div>;
}

function Shareholders({ houseId, village, onMessage }: { houseId: string; village: VillageState; onMessage: (message: string) => void }) {
  ...
  return <div className="operation-stack"><div className={`allocation-total ${total === 10_000 ? 'valid' : 'invalid'}`}>分潤比例合計 <strong>{(total / 100).toFixed(2)}%</strong>{total !== 10_000 && <small>建立分潤前必須剛好 100%</small>}</div>{holdings.map((holding) => { const person = village.shareholders.find((row) => row.id === holding.shareholderId); return <div className="holding-row" key={holding.id}><span>{person?.displayName ?? '未知股東'}<small>{person?.referenceCode}</small></span><strong>{(holding.profitShareBasisPoints / 100).toFixed(2)}%</strong><i>{holding.syncStatus}</i></div>; })}<form className="inline-operation-form" onSubmit={submit}><h3>新增股東持股草稿</h3><label>顯示名稱<input required name="name" /></label><label>分潤比例（%）<input required min="0.01" max="100" step="0.01" type="number" name="percent" /></label><button className="secondary-button">保存本機草稿</button></form></div>;
}

function Distributions({ houseId, village, online, run }: { houseId: string; village: VillageState; online: boolean; run: (action: () => void, success: string) => void }) {
...
function EditHouse({ house, village, onMessage }: { house: ChickenHouse; village: VillageState; onMessage: (message: string) => void }) {
  function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const data = new FormData(event.currentTarget); village.updateHouse(house.id, { name: String(data.get('name')), species: String(data.get('species')) as ChickenHouse['species'], designCapacity: Number(data.get('capacity')), currentBirdCount: Number(data.get('birds')) }); onMessage('雞舍修改已保存並排入同步。'); }
  return <div className="operation-stack"><form className="inline-operation-form" onSubmit={submit}><label>雞舍名稱<input required name="name" defaultValue={house.name} /></label><SpeciesSelect /><label>設計容量<input required min="1" type="number" name="capacity" defaultValue={house.designCapacity} /></label><label>當前在養<input required min="0" type="number" name="birds" defaultValue={house.currentBirdCount} /></label><button className="primary-button">保存修改</button></form><button className="danger-button archive-button" onClick={() => { village.archiveHouse(house.id); onMessage('雞舍已封存；歷史資料與稽核紀錄仍保留。'); }}>封存雞舍</button></div>;
}
