...
export function ManagerPage({ visits, signedIn, isAdmin, onEquip }: { visits: VisitProgress; signedIn: boolean; isAdmin: boolean; onEquip: (slot: 'head' | 'body' | 'hand' | 'back', id: string | null) => void }) {
  ...
  return (
    <div className="page manager-page">
      <section className="manager-stage illustrated-plate"><img src={assetUrl('assets/art/vanadis-role-chronicle.webp')} alt="管理者、代養戶、行情商人、孵化師、巡查員與帳房員的人物設定圖鑑" /><div className="manager-stage__copy"><p className="eyebrow">{isAdmin ? '人物圖鑑・管理者卷' : '人物圖鑑・村民卷'}</p><h1>晨巡裝備室</h1><p>{isAdmin ? '管理者使用專用外觀，登入校錄與行裝穿搭會保存至 Firebase。' : '每位使用者皆可使用紙娃娃行裝；系統會依台北日期每日校錄一次，同日重複開啟不會重複計數。'}</p>{signedIn ? <span className="login-recorded-badge">{isAdmin ? '管理者外觀・今日已校錄' : '村民行裝・今日已校錄'}</span> : <Link className="primary-button" to="/admin">登入以記錄今日</Link>}</div><div className="manager-stage__avatar"><ManagerAvatar equipped={visits.equipped} role={isAdmin ? 'admin' : 'resident'} /><small>{isAdmin ? '管理者專用外觀・行裝即時預覽' : '村民紙娃娃・行裝即時預覽'}</small></div></section>
