  ...
  return (
    <div className="page home-page">
      <section className="home-hero">
        ...
        <div className="home-hero__copy"><p className="eyebrow">清晨巡村手冊</p><h1>今天也把雞情<br />照顧得明明白白</h1><DataBadge tone={bundle.mode === 'live' ? 'live' : 'fixture'}>{bundle.mode === 'live' ? '正式來源' : '已驗證快照'}</DataBadge></div>
      </section>
      <PixelPanel title="今日行情摘要" action={<Link to="/today">查看全部 →</Link>}>
      ...
      <div className="rpg-menu">{menu.map(([to, label, description], index) => <Link to={to} key={to}><b>{String(index + 1).padStart(2, '0')}</b><span><strong>{label}</strong><small>{description}</small></span><i>›</i></Link>)}</div>
