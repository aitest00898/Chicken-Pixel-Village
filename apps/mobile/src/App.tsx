  ...
  return (
    <AppShell offline={!online} syncLabel={syncing ? '正在確認最新行情…' : village.unsyncedCount ? `本機草稿 ${village.unsyncedCount} 筆待同步` : bundle.mode === 'live' ? '正式行情已同步' : '使用已驗證快照'}>
      <Routes>
        ...
        <Route path="/village" element={<VillagePage houses={village.houses} placements={village.mapPlacements} onMove={village.moveHouse} />} />
        <Route path="/houses" element={privatePage(<HousesPage village={village} online={online} />)} />
        <Route path="/manager" element={<ManagerPage visits={village.visits} onVisit={village.visitToday} onEquip={village.equip} />} />
        <Route path="/settings" element={<SettingsPage dark={dark} reduced={reduced} onDark={setDark} onReduced={setReduced} mode={bundle.mode} storageMode={village.storageMode} authLabel={authentication.userLabel} onSignOut={authentication.signOut} online={online} unsyncedCount={village.unsyncedCount} syncMode={village.syncMode} syncError={village.syncError} onSync={() => { void village.syncNow(online); }} />} />
